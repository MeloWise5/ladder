...
export const snapshotBreakdownChartAction = (date_method = 'all') => async (dispatch, getState) => {
    try {
        dispatch({ type: SNAPSHOT_BREAKDOWN_REQUEST })
        ...
        const { data } = await axios.get(`/api/snapshot/chart/breakdown/all?date_method=${date_method}`, config)
        dispatch({ type: SNAPSHOT_BREAKDOWN_SUCCESS, payload: data })
    } catch (error) {
        dispatch({
            type: SNAPSHOT_BREAKDOWN_FAIL,
            payload: error.response && error.response.data.detail ?
                error.response.data.detail : error.message,
        })
    }
